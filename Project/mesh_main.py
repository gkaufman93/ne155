...
def main(inf, outd, flag, sflag, gflag):
    ...
    with open(inf,'r') as f:
        ...
        while len(sr) > 0 and sr[0] != 'M':
            ...
            if not all(float(i) >= 0 for i in l):
                raise LessThanZeroError('One or more arguments have values that are '\
                                    +'less than zero, line {}'.format(line_num))
            elif cell_xmax <= cell_xmin or cell_ymax <= cell_ymin or \
                    cell_xmax > W or cell_ymax > H:
        ...
        while sr[0] != 'S':
            ...
            if not all(float(i) > 0 for i in l):
                raise LessThanZeroError('One or more arguments have values that '\
                                    +'are less than zero, line {}'.format(line_num))
        ...
        while len(sr) > 0:
            ...
            if (sx_min >= sx_max) or (sy_min >= sy_max) or (sx_max > Nx) or (sy_min > Ny):
                raise BoundError('Bounds are nonsensical, line '\
                                 +'{}'.format(line_num))
    ...
    if sflag:
        ...
        xs = np.tile(np.linspace(0,W,Nx+1),Ny+1)-d/2
        ys = np.repeat(np.linspace(0,H,Ny+1),Nx+1)-e/2
        Hs,xedge,yedge =\
            np.histogram2d(xs,ys,bins=[Nx,Ny],range=[[0,W],[0,H]],weights=Smat)
        Hs = np.rot90(Hs)
        Hs = np.flipud(Hs)
        fig1 = pp.figure()
    ...
    if flag:
        ...
        for ind in range(len(F)):
            ...
            log_str += '\n'
        log_str += '\n\n'
    # Graph flux output in heatmap, if flagged.
    if gflag:
        print 'Plotting flux as heat map...'
        F = F.reshape(len(F))
        xf = np.tile(np.linspace(0,W,Nx+1),Ny+1)-d/2
        yf = np.repeat(np.linspace(0,H,Ny+1),Nx+1)-e/2
        Hf,xedge,yedge =\
            np.histogram2d(xf,yf,bins=[Nx,Ny],range=[[0,W],[0,H]],weights=F)
        Hf = np.rot90(Hf)
        Hf = np.flipud(Hf)
        fig2 = pp.figure()
...
def func(argkey, x, y, args):
    if argkey == 'CONST':
        ...
        return abs(args[0]*cos(pi*args[1]*x + pi*args[2]) + args[3])
    elif argkey == 'COS_Y':
        return abs(args[0]*cos(pi*args[1]*y + pi*args[2]) + args[3])
    else:
